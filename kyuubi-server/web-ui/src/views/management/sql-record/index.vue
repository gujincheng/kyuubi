  ...
  <main class="sql-record-page">
      ...
      <el-form :inline="true" @submit.prevent="search">
        ...
        <el-form-item>
          <el-input
            ...
            clearable
            :placeholder="$t('sql_record.session')"
            @keyup.enter="search" />
        </el-form-item>
        <el-form-item>
          <el-input
            ...
            clearable
            :placeholder="$t('sql_record.engine')"
            @keyup.enter="search" />
        </el-form-item>
        <el-form-item>
          <el-select
    ...
    <el-card class="records-card" shadow="never">
      <div class="card-heading">
        <div>
          <span class="heading-kicker">LIVE BUFFER</span>
          <h2>{{ $t('sql_record.title') }}</h2>
        </div>
      ...
      <el-table
        ...
        class="records-table"
        row-key="id">
        <el-table-column :label="$t('sql_record.statement')" min-width="360">
        ...
        <el-table-column :label="$t('sql_record.state')" width="130">
          <template #default="scope">
            <el-tag :type="stateTagType(scope.row.state)" effect="light" round>
        ...
        <el-table-column :label="$t('sql_record.created')" width="165">
          <template #default="scope">{{
            formatTime(scope.row.createTime)
          }}</template>
        </el-table-column>
        <el-table-column :label="$t('sql_record.queue_wait')" width="125">
          <template #default="scope">{{
            formatDuration(scope.row.queueWaitTimeMs)
          }}</template>
        </el-table-column>
        <el-table-column :label="$t('sql_record.execution_time')" width="130">
          <template #default="scope">{{
            formatDuration(scope.row.executionDurationMs)
          }}</template>
        </el-table-column>
      ...
      <el-empty
        ...
        :description="$t('sql_record.no_records')">
      ...
      <template v-if="selectedRecord">
        ...
        <div class="detail-grid">
          ...
          <div
            ...
            ><strong>{{ formatTime(selectedRecord.completeTime) }}</strong></div
          >
          <div
            ><label>{{ $t('sql_record.queue_wait') }}</label
            ><strong>{{
              formatDuration(selectedRecord.queueWaitTimeMs)
            }}</strong></div
          >
          <div
            ><label>{{ $t('sql_record.execution_time') }}</label
            ><strong>{{
              formatDuration(selectedRecord.executionDurationMs)
            }}</strong></div
          >
          <div
        ...
        <section class="detail-section timing-section">
          ...
          <div class="execution-timeline">
            ...
            <div class="timeline-item">
              <span class="timeline-dot waiting" />
              <div>
                <strong>{{ $t('sql_record.queued') }}</strong>
                <small>{{
                  formatDuration(selectedRecord.queueWaitTimeMs)
                }}</small>
              </div>
        ...
        <section class="detail-section error-section">
          ...
          <pre>{{
            selectedRecord.errorMessage || $t('sql_record.no_error_detail')
          }}</pre>
        </section>
      </template>
    </el-drawer>
...
<script setup lang="ts">
  ...
  import {
    getSqlExecutionRecords,
    type SqlExecutionRecord,
    type SqlExecutionRecordQuery
  } from '@/api/sql-record'
  ...
  const records = ref<SqlExecutionRecord[]>([])
  const total = ref(0)
  const page = ref(1)
  const pageSize = ref(20)
  const detailVisible = ref(false)
  const selectedRecord = ref<SqlExecutionRecord | null>(null)
  ...
  const query = (): SqlExecutionRecordQuery => ({
    page: page.value,
    ...
    fromTime: filters.timeRange?.[0]?.getTime(),
    toTime: filters.timeRange?.[1]?.getTime()
  })
  ...
  const loadRecords = async () => {
    ...
    loading.value = true
    try {
      const result = await getSqlExecutionRecords(query())
  ...
  const exportRecords = async () => {
    ...
    try {
      const result = await getSqlExecutionRecords({
        ...query(),
        page: 1,
        pageSize: 200
      })
      ...
      link.href = url
      link.download = `sql-records-${format(new Date(), 'yyyyMMdd-HHmmss')}.csv`
      link.click()
      link.remove()
  ...
  const totalDuration = (record: SqlExecutionRecord) => {
  ...
  const durationHint = (record: SqlExecutionRecord) =>
  ...
  const showDetail = (record: SqlExecutionRecord) => {
